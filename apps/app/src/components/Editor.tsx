		...
		<div className="flex h-full bg-ink-950 text-ink-50 font-mono">
			...
			<aside className="flex w-72 shrink-0 flex-col gap-6 border-r border-ink-700 p-5 overflow-y-auto">
				<header className="flex items-center justify-between">
					<span className="text-xs text-ink-300 tracking-widest uppercase">Halftone Studio</span>
				...
				<section className="flex flex-col gap-3">
					...
					<div className="flex gap-3">
						<ColorPicker label="Dark" value={darkColor} onChange={(v) => { setDarkColor(v); resetReady(); }} />
						<ColorPicker label="Light" value={lightColor} onChange={(v) => { setLightColor(v); resetReady(); }} />
					</div>
				</section>
				...
				<section className="flex flex-col gap-4">
					...
					<Slider
						...
						step={1}
						format={(v) => String(v)}
						onChange={(v) => { setCellW(v); resetReady(); }}
					/>
					<Slider
						label="Cell Height"
						value={cellH}
						...
						step={1}
						format={(v) => String(v)}
						onChange={(v) => { setCellH(v); resetReady(); }}
					/>
					<Slider
						label="Gamma"
						value={gamma}
						...
						onChange={(v) => { setGamma(v); resetReady(); }}
					/>
				</section>
